...
export default function MyTeamContentPage({ countries, seasons }: IMyTeamContentPageProps): JSX.Element {
    const { token } = useAuth()
    const [selectedFiltersAndResults, setSelectedFiltersAndResults] = useState<ISelectedFilter>({
        country: null,
        league: null,
        season: null,
        team: null,
    ...
    const handleSelectSeason = useCallback(async (season: number): Promise<void> => {
        ...
        try {
            ...
            loadedLeagues.current = response.response.map(item => item.league)
    ...
    const handleSelectLeague = useCallback(async (league: ILeague): Promise<void> => {
        setSelectedFiltersAndResults((prev) => {
            return {
                ...prev,
                league,
        ...
        if (inputSearchLeagueRef.current && inputSearchLeagueRef.current.value) {
        ...
        try {
            const response = await fetcher({
                ...
                method: 'GET',
                queryParams: {
                    country: selectedFiltersAndResults.country?.name as string,
    ...
    const handleSearchLeague = useCallback((event: React.ChangeEvent<HTMLInputElement>): void => {
        const search = event.target.value.toLowerCase()
        const filtered = loadedLeagues.current.filter(league => league.name.toLowerCase().includes(search))
        setFilteredLeagues(filtered)
    }, [])

    const handleSelectTeam = useCallback(async (team: ITeam): Promise<void> => {
        if (inputSearchTeamRef.current && inputSearchTeamRef.current.value) {
        ...
        try {
            const [playersResponse, teamStatisticsResponse]: [IFetchResponsePlayers, IFetchResponseTeamStatistics] = await Promise.all([
                fetcher({
                    url: '/players',
                    method: 'GET',
                    queryParams,
            ...
            const goalsAggregatedToArray = Object.keys(goalsAggregated).map((key) => {
                return {
                    minute: key + ' min',
    ...
    const handleSearchTeam = useCallback((event: React.ChangeEvent<HTMLInputElement>): void => {
        const search = event.target.value.toLowerCase()
        const filtered = loadedTeams.current.filter(team => team.name.toLowerCase().includes(search))
                    ...
                    <div className='grid grid-cols-3 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3'>
                        {filteredSeasons.map((season) => (
                            <SeasonCard
                                ...
                                season={season}
                                selected={selectedFiltersAndResults.season === season}
                            />
                        ))}
                    </div>
                </div>
                    ...
                    <div className='grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3'>
                        {filteredLeagues.map((league) => (
                            <LeagueCard
                                ...
                                selected={selectedFiltersAndResults.league?.id === league.id}
                            />
                        ))}
                    </div>
                </div>
                    ...
                    <div className='grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3'>
                        {filteredTeams.map((team) => (
                            <TeamCard
                                ...
                                selected={selectedFiltersAndResults.team?.id === team.id}
                                team={team}
                            />
                        ))}
                    </div>
